...
def post_fic(fic_data, id, client, state="published"):
    ...
    slug = ''.join([char for char in fic_data['Title'] if char in 'abcdefghijklmnopqrstuvwxyz-']) + "_" + id
    ...
    client.create_text('https://sjficlist-dev.tumblr.com', state=state, slug=slug, body=post_body, format='html', tags=tags)

    return slug



client = init_tumblr_client()
print(client.info())
